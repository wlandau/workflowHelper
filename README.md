...
If you have the `rmarkdown` and `tools` packages, you can run this example from start to finish with the `run_example_workflowHelper` function. Alternatively, you can set up earlier stages with `write_example_workflowHelper` or `setup_example_workflowHelper` and then run the output files by hand. Optionally, use the `clean_example_workflowHelper` function to remove all the files generated by `run_example_workflowHelper`. The details of the example are below. 

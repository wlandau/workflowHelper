...
- `make clean` removes all the files generated by `make`. If some of your files are produced by side effects, `make clean` might not remove them. In that case, use the `clean` argument of `plan_workflow` to add more shell commands to the `clean` rule. For example, 
```{r}

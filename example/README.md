...
- `make clean` removes all the files generated by `make`. If some of your files are produced by side effects, `make clean` might not remove them. In that case, use the `clean` argument of `write_makefile` to add more shell commands to the `clean` rule. For example, 
```{r}
plan_workflow(stages, clean = c("rm -f output1.csv", "rm -f plot5.pdf"))
```

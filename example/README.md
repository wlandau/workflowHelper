...
where the `commands` function parses named expressions. Currently, the `commands` function is like `parallelRemake::strings` with some error checking, so I could have alternatively written
